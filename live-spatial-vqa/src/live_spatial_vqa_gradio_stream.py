#!/usr/bin/env python3
"""
Gradio Web Interface for Live Spatial VQA with Auto-Streaming
Browser-based interface with automatic camera stream updates
"""
import gradio as gr
import cv2
import numpy as np
...
class GradioStreamingSpatialVQA:
    """Gradio-based web interface with auto-streaming for live spatial VQA"""
    ...
    def get_stats(self) -> str:
        ...
        if self.camera:
            output += f"\n📹 Camera Info\n"
            output += f"Status: {'Running' if self.camera.running else 'Stopped'}\n"
            output += f"Resolution: {self.camera.width}x{self.camera.height}\n"
            output += f"Frame rate: {self.camera.get_fps():.1f} FPS\n"
    ...
    def create_interface(self) -> gr.Blocks:
        """Create Gradio interface with auto-streaming"""
        
        with gr.Blocks(title="Live Spatial VQA - Streaming", theme=gr.themes.Soft()) as interface:
            gr.Markdown("""
            # 🎥 Live Spatial VQA System (Auto-Streaming)
            Real-time spatial reasoning from camera streams using SpaceOm and SpaceThinker
            
            **✨ Camera stream updates automatically when you click "🔄 Update Stream"!**
            """)
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        ...
                        refresh_btn = gr.Button("🔄 Update Stream", variant="secondary")
            ...
            # Load initial camera frame when interface starts
            interface.load(
                fn=self.get_current_frame,
                outputs=camera_image
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Gradio Streaming Web Interface for Live Spatial VQA")
    ...
    print("🌐 Starting Gradio streaming web interface...")
    print(f"   Camera: {args.camera}")
    print(f"   Model directory: {args.model_dir}")
    print(f"   Port: {args.port}")
    print("   ✨ Auto-streaming enabled (1 FPS)")
