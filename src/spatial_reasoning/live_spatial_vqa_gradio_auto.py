#!/usr/bin/env python3
"""
Gradio Web Interface for Live Spatial VQA with True Auto-Streaming
Browser-based interface with continuous camera stream updates using generator
"""
import gradio as gr
import cv2
import numpy as np
...
class GradioAutoStreamSpatialVQA:
    """Gradio-based web interface with true auto-streaming for live spatial VQA"""
    ...
    def initialize_system(self) -> Tuple[str, str]:
        ...
        try:
            ...
            status += f"📹 Camera: {self.camera.get_info()['status']}\n"
            status += f"🧠 Model: {current_model} loaded\n"
            status += f"🎯 Available models: {', '.join(available_models.keys())}"
    ...
    def stream_frames(self) -> Generator[Image.Image, None, None]:
        """Generator that yields camera frames for streaming"""
        self.streaming_active = True
        while self.streaming_active:
            frame = self.get_current_frame()
            if frame:
                yield frame
            time.sleep(0.5)  # Update every 500ms for web interface
    ...
    def ask_question(self, question: str) -> Tuple[str, str]:
        ...
        try:
            ...
            # Process question
            result = self.inference_engine.process_question(question, current_frame)
            
            if result['success']:
                answer = result['answer']
                metadata = f"Model: {result['model']}\n"
                metadata += f"Processing time: {result['processing_time']:.2f}s\n"
                metadata += f"Frame size: {current_frame.size}\n"
                metadata += f"Timestamp: {result['timestamp']}"
    ...
    def get_stats(self) -> str:
        ...
        if self.camera:
            ...
            output += f"\n📹 Camera Info\n"
            output += f"Status: {camera_info['status']}\n"
            output += f"Resolution: {camera_info['resolution']}\n"
            output += f"Frame rate: {camera_info['fps']}\n"
    ...
    def create_interface(self) -> gr.Blocks:
        """Create Gradio interface with streaming video"""
        
        with gr.Blocks(title="Live Spatial VQA - Auto Stream", theme=gr.themes.Soft()) as interface:
            gr.Markdown("""
            # 🎥 Live Spatial VQA System (True Auto-Streaming)
            Real-time spatial reasoning from camera streams using SpaceOm and SpaceThinker
            
            **✨ Features continuous video streaming! Click "🎬 Start Stream" to begin.**
            """)
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        ...
                        stream_btn = gr.Button("🎬 Start Stream", variant="secondary")
            ...
            stream_btn.click(
                fn=self.start_streaming,
                outputs=camera_image
            )
            
            stop_btn.click(
                fn=self.stop_streaming,
                outputs=camera_image
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Gradio Auto-Streaming Web Interface for Live Spatial VQA")
    ...
    # Create application
    app = GradioAutoStreamSpatialVQA(
    ...
    print("🌐 Starting Gradio auto-streaming web interface...")
    print(f"   Camera: {args.camera}")
    print(f"   Model directory: {args.model_dir}")
    print(f"   Port: {args.port}")
    print("   ✨ Continuous streaming enabled")
